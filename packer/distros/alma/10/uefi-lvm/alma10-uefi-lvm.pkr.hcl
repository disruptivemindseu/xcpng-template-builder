...
variable "disk_name" {
  ...
  default     = "template-alma10-uefi-lvm_disk1"
}
...
locals {
  ...
  vm_name        = coalesce(var.vm_name, "template-alma10-uefi-lvm_${local.timestamp}")
  vm_description = coalesce(var.vm_description, "[Template] alma 10 UEFI LVM built on ${local.buildtime} by Packer")
}
