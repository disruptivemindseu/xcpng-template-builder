...
variable "disk_name" {
  ...
  default     = "template-centos10-uefi-lvm_disk1"
}
...
locals {
  ...
  vm_name        = coalesce(var.vm_name, "template-centos10-uefi-lvm_${local.timestamp}")
  vm_description = coalesce(var.vm_description, "[Template] CentOS 10 UEFI LVM built on ${local.buildtime} by Packer")
}
...
source "xcp-iso" "template" {
  ...
  ssh_password           = "centos10-uefi-lvm"
