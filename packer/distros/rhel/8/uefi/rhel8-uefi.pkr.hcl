...
variable "disk_name" {
  ...
  default     = "template-rhel8-uefi_disk1"
}
...
locals {
  ...
  vm_name        = coalesce(var.vm_name, "template-rhel8-uefi_${local.timestamp}")
  vm_description = coalesce(var.vm_description, "[Template] RHEL 8 UEFI built on ${local.buildtime} by Packer")
}
