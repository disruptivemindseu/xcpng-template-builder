...
variable "disk_name" {
  ...
  default     = "template-rhel9-uefi-lvm_disk1"
}
...
locals {
  ...
  vm_name        = coalesce(var.vm_name, "template-rhel9-uefi-lvm_${local.timestamp}")
  vm_description = coalesce(var.vm_description, "[Template] RHEL 9 UEFI LVM built on ${local.buildtime} by Packer")
}

source "xenserver-iso" "template" {
  iso_name     = "rhel-9.8-x86_64-dvd.iso"
