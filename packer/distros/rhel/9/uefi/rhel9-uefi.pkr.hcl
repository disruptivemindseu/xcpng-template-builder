...
variable "disk_name" {
  ...
  default     = "template-rhel9-uefi_disk1"
}
...
locals {
  ...
  vm_name        = coalesce(var.vm_name, "template-rhel9-uefi_${local.timestamp}")
  vm_description = coalesce(var.vm_description, "[Template] RHEL 9 UEFI built on ${local.buildtime} by Packer")
}
