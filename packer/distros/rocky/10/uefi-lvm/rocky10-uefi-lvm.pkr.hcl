...
variable "disk_name" {
  ...
  default     = "template-rocky10-uefi-lvm_disk1"
}
...
locals {
  ...
  vm_name        = coalesce(var.vm_name, "template-rocky10-uefi-lvm_${local.timestamp}")
  vm_description = coalesce(var.vm_description, "[Template] rocky 10 UEFI LVM built on ${local.buildtime} by Packer")
}

source "xenserver-iso" "template" {
  iso_name       = "Rocky-10.2-x86_64-dvd1.iso"
