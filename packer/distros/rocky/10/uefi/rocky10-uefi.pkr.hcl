...
variable "disk_name" {
  ...
  default     = "template-rocky10-uefi_disk1"
}
...
locals {
  ...
  vm_name        = coalesce(var.vm_name, "template-rocky10-uefi_${local.timestamp}")
  vm_description = coalesce(var.vm_description, "[Template] rocky 10 UEFI built on ${local.buildtime} by Packer")
}
